...
##
## Эта компонента содержит реализацию обычного движения (ходьба или бег), 
## падения, прыжка, скольжения по стенам и прыжка от стен для игрока. В качестве
## параметров используются данные из [PlayerMovementData].
